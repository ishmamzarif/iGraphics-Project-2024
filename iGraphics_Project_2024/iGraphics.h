...
AUX_RGBImageRec* loadBMP(const char filename[]) {
    // Load the BMP file and return a pointer to the image data
    return auxDIBImageLoad(filename);
}

void iShowLoadedBMP2(int x, int y, AUX_RGBImageRec *TextureImage, int ignoreColor) {
    if (!TextureImage) return;  // Ensure TextureImage is valid
...
void iShowLoadedBMP(int x, int y, AUX_RGBImageRec *TextureImage){
    iShowLoadedBMP2(x, y, TextureImage, -1);
}
